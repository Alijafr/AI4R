...
def q4_move(self, motion,tolerance = 0.001):
    ...
    if abs(steering2) < tolerance: 
        
        x_new = self. x + distance2*math.cos(self.orientation)
        y_new = self.y + distance2*math.sin(self.orientation)
        theta_new = self.orientation 
    else:
        r = self.length/math.tan(steering2)
        beta = distance2/r 
        x_dis = math.sin(self.orientation) * r
        y_dis = math.cos(self.orientation) * r
        center_x = self.x - x_dis
        center_y = self.y + y_dis


        x_dist_new = math.sin(self.orientation+beta) * r
        y_dist_new = math.cos(self.orientation+beta) * r
        x_new = center_x + x_dist_new
        y_new = center_y - y_dist_new
        theta_new = (self.orientation + beta)%(2*math.pi)
...
def q6_move(self, motion,tolerance = 0.001):
    # You can replace the INSIDE of this function with the move function you modified in the module quiz
