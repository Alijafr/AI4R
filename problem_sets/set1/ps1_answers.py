...
def move(p, motion, p_move):
    #total probability 
    p_stay = 1. - p_move 
    aux = [[0. for _col in range(len(p[0]))] for _row in range(len(p))]

    for i in range(len(aux)):
        for j in range(len(aux[0])):
            aux[i][j] = p[(i-motion[0]) % len(aux)][(j-motion[1])%len(aux[0])]*p_move + p[i][j]*p_stay  
...
def sense(p,colors, measurment, sensor_right):
    ...
    for i in range(len(aux)):
        for j in range(len(aux[0])):
            ...
            Z = colors[i][j] == measurment
            aux [i][j] = p[i][j] * (sensor_right*Z + sensor_worng*(1-Z))
            sum += aux[i][j]
