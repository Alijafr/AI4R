...
class Meteorite(object):
    ...
    def xy_pos(self, t):
        ...
        x_pos = ((self.c_pos_x * t_shifted * t_shifted)
                 + (self.c_vel_x * t_shifted)
                 + self.c_accel * self.accel_factor)

        y_pos = ((self.c_pos_y * t_shifted * t_shifted)
...
class MeteorShower(object):
    ...
    def check_for_laser_hit(self, time, laser_line, laser_hdg_rad,
        ...
        if laser_shots_remaining <= 0:
            return False
        if distance_formula([self.turret.x_pos, self.turret.y_pos],
