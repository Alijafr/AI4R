...
class DualRotor(object):
    ...
    def __init__(self):
        ...
        self.x = 0 # for now assume only vertical movement is possible
        ...
        # speed
        self.WIND_FACTOR_STD = 0.05 # std dev for wind
        self.WIND_FACTOR = 0.0 #random.gauss(0, self.WIND_FACTOR_STD)
    ...
    def calc_one_side_move(self, id, dt):
        ...
        # calculate movement
        new_y = self.y + self.velocity * dt + (1/2) * a * dt**2
        ...
        # For now 0 roll angle is pointing right (along x-axis)
    ...
    def move(self, dt=1/10):
        ...
        if propeller_speeds_same:
            ...
            #self.velocity = self.velocity - self.g*dt #(new_y - self.y) / dt
            ...
            self.y = 0.5 * (new_y1 + new_y2) # center point of the drone, approx as average of each side's y
            ...
            self.x += x_disp_1 + x_disp_2
            
            #add wind factor but only if drone has lift off
            if self.y > 0:
                self.x += self.WIND_FACTOR
    ...
    def update_rotor_speed(self, thrust_chg, roll_chg):
        ...
        # Clip roll change to max roll change allowed per step
        if roll_chg != 0:
