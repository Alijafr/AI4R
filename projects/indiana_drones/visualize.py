...
class TurtleDisplay(object):
    ...
    def tree_at_location(self, tree_id: str, x: float, y: float, r: float):
    ...
    try:
        ...
        for move in params['move']:
            ...
            for tree in state.tree_locs_on_map:
                display.tree_at_location(tree['type'], tree['x'], tree['y'], tree['radius'])
            ...
            for location in state.tree_locs_on_map:
                co_ordinates = rover_slam.get_coordinates()
                if len(co_ordinates) > 0:
                    x, y = co_ordinates[location['id']]
